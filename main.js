import "./style.css";
import * as quizElements from "./mockData.js";
...
function marcarRespuesta(btn) {
  quizElements.quizElements[questionCounter]["respuesta"] = btn.textContent;
...
function marcarCorrecta() {
  ul1.childNodes.forEach((li) => {
    if (
      li.firstChild.textContent == quizElements.quizElements[questionCounter]["respuesta"]
    ) {
...
function generarRespuestas(indice) {
  ...
  let counter = 0;
  quizElements.quizElements[indice]["answers"].forEach((answer) => {
    ...
    btn.addEventListener("click", () => {
      ...
      if (!todosTienenRespuesta) {
        todosTienenRespuesta = quizElements.quizElements.every((elemento) =>
...
p1.textContent = quizElements.quizElements[questionCounter]["question"];
...
const modalNuevo = DOMHelper.crearModal();
body.append(modalNuevo);
...
if (p1.textContent == quizElements.quizElements[0]["question"]) {
...
btnPreview.addEventListener("click", () => {
  if (questionCounter == quizElements.quizElements.length - 1) {
...
btnNext.addEventListener("click", () => {
  ...
  generarRespuestas(questionCounter);
  marcarCorrecta();
  if (p1.textContent == quizElements.quizElements.at(-1)["question"]) {
...
btnCheck.addEventListener("click", () => {
  ...
  quizElements.quizElements.forEach((elemento) => {
...
function mostrarModal(respuestasCorrectas) {
    const divModalExt = document.querySelector(".modal");
    const pModalText = document.querySelector("#p_aciertos");
  pModalText.textContent =
    respuestasCorrectas + " correct answers from " + quizElements.quizElements.length;
