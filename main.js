...
function marcarCorrecta() {
  ul1.childNodes.forEach((li) => {
    if (
      li.firstChild.textContent == quizElements[questionCounter]["respuesta"]
    ) {
...
btnCheck.addEventListener("click", () => {
  ...
  crearModal(respuestasCorrectas);
});

function crearModal (respuestasCorrectas){
  ...
  h2Modal.textContent = "RESULTADO";
  const hrModal = document.createElement("hr");
  const pModalText = document.createElement("p");
  pModalText.textContent = respuestasCorrectas + " correct answers from " + quizElements.length;

  
  divModalInt.append(pModalX,h2Modal,hrModal,pModalText);
